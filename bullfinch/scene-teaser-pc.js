...
(function () {
  ...
  // TEASER fork: a stronger forward push so the descent reads as flying INTO
  // the mote field and rising into the assembled tree cloud (vs the investor
  // build's gentler pan). Start further back, land closer in.
  var CAM_START = new THREE.Vector3(0, 4.2, 9.4);
  var CAM_END   = new THREE.Vector3(0, 0.7, 1.7);
  ...
  // TEASER fork: more motes so the field reads dense (less spacey) and the
  // grove fills in richer. More particles also = more tree fill points.
  var PARTICLE_POOL = mobileLike ? 420 : 640;
  ...
  // TEASER fork: tighter horizontal spread so the motes cluster closer (less
  // spacey) instead of scattering to the edges.
  var FIELD_HALF_X = Math.max(2.0, Math.min(4.4, 3.2 * viewAspect));
  ...
  function measureGroveZone() {
    // TEASER fork: the grove is CENTRED. The three trees sit at the
    // bottom-centre of the §03 screen with the compressed copy + logos
    // overlapping their crowns, rather than parked left of a text column.
    var w = window.innerWidth;
    var narrow = w < 720;
    groveZoneL = narrow ? w * 0.14 : w * 0.30;
    groveZoneR = narrow ? w * 0.86 : w * 0.70;
  }
  measureGroveZone();
  ...
  function setProgress(p) {
    ...
    // TEASER fork: a density FLOOR so the very first screen already shows a low,
    // present mote layer (we start inside the field), then it thickens as we fly
    // forward into the grove.
    uniforms.uMoteDensity.value = 0.24 + 0.76 * v;
    uniforms.uConnect.value     = v;
  }
  // TEASER fork: day -> night descent. Lerps the renderer clear colour, the
  // backdrop grade, and the mote colour from the daylight CSS values toward a
  // night sky, so by the close the motes read as a bright starfield on dark and
  // the convergence lands as a constellation. Driven from teaser.html by global
  // scroll progress; contrast is preserved because bg + motes move together.
  var _nightDayCaptured = false;
  var _dayClear, _dayCanopy, _dayUnder, _dayMote;
  var NIGHT_CLEAR  = new THREE.Color(0x0b0f16);
  var NIGHT_CANOPY = new THREE.Color(0x121a2c);
  var NIGHT_UNDER  = new THREE.Color(0x05070b);
  var NIGHT_MOTE   = new THREE.Color(0xe9edf6);
  function setNightProgress(p) {
    var v = Math.max(0, Math.min(1, p));
    if (!_nightDayCaptured) {
      _dayClear  = bgColor.clone();
      _dayCanopy = uniforms.uCanopy.value.clone();
      _dayUnder  = uniforms.uUnderstory.value.clone();
      _dayMote   = partMat.uniforms.uColor.value.clone();
      _nightDayCaptured = true;
    }
    bgColor.copy(_dayClear).lerp(NIGHT_CLEAR, v);
    renderer.setClearColor(bgColor, 1);
    uniforms.uCanopy.value.copy(_dayCanopy).lerp(NIGHT_CANOPY, v);
    uniforms.uUnderstory.value.copy(_dayUnder).lerp(NIGHT_UNDER, v);
    partMat.uniforms.uColor.value.copy(_dayMote).lerp(NIGHT_MOTE, v);
  }
  ...
  window.bullfinchCanopy = {
    ...
    setNightProgress: setNightProgress,
